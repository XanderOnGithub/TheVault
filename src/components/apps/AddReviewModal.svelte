<script>
	...
	import { addReviewToApp } from '../../lib/firebase/firestoreService';
	...
	const handleSubmit = async () => {
		...
		const success = await addReviewToApp(appId, userId, rating, reviewText);
		if (success) {
			dispatch('close');
		} else {
...
<div class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
	...
	>
		<h2 class="text-2xl font-bold mb-4 text-accent">Add Review</h2>
		<div class="mb-4">
